function [Y]=PRI(x,beta,iter,sig)
...
% [2] Sudhir Madhav Rao, Unsupervised learning: An information theoretic framework, University of Florida.
...
%     end
    ...
    for k=1:N
       ...
       x_1(k,:)=c*(1-beta)/beta* sum(num_1)/sum(den_+0.000000001)+ sum(num_)/sum(den_+0.0000000001)-c*(1-beta)/beta*sum(num_2)/sum(den_+0.0000000001); %corresponding to Eq. 12 in the reference [1].
    end
    x=x_1; 
end
end
